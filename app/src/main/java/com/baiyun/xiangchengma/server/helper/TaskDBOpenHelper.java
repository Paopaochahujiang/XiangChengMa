...
import com.baiyun.xiangchengma.bean.Task;
...
public class TaskDBOpenHelper extends SQLiteOpenHelper {
    ...
    public ArrayList<Task> getAllData(){//把表中所有数据放到list中
        ...
        while(cursor.moveToNext()){
            ...
            @SuppressLint("Range") String task_abstract=cursor.getString(cursor.getColumnIndex("abstract"));
            @SuppressLint("Range") String detail=cursor.getString(cursor.getColumnIndex("detail"));
            @SuppressLint("Range") String iffinished=cursor.getString(cursor.getColumnIndex("iffinished"));
